...
import styles from "./animated-more-reviews-button.module.css"
import { useState } from "react"

export default function AnimatedMoreReviewsButton() {
  const [isHovered, setIsHovered] = useState(false)
  const [isClicked, setIsClicked] = useState(false)

  const text1 = "Voir plus d'avis".split("")
  const text2 = "Google".split("")

  return (
    <Link
      href="https://share.google/eolJZMgRvJdaFGLmi"
      target="_blank"
      ...
      onFocus={() => setIsHovered(true)} // For keyboard navigation
      onBlur={() => setIsHovered(false)} // For keyboard navigation
    >
      ...
      <div className={styles.wrap}>
        ...
        <div className={styles.content}>
          <span className={`${styles.char} ${isHovered ? styles["state-2"] : styles["state-1"]}`}>
            {text1.map((char, i) => (
              <span key={i} data-label={char} style={{ "--i": i + 1 } as React.CSSProperties}>
                {char}
              </span>
            ))}
          </span>
          ...
          <span className={`${styles.char} ${isHovered ? styles["state-1"] : styles["state-2"]}`}>
            {text2.map((char, i) => (
              <span key={i} data-label={char} style={{ "--i": i + 1 } as React.CSSProperties}>
                {char}
              </span>
